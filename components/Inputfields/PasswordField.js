...
export function PasswordField({ name, label, type, ...props }) {
  const {
    register,
    formState: { errors },
  } = useFormContext();
    ...
    <div className="relative">
      <label htmlFor={name} className="cursor-pointer">
        ...
        <div className="mt-2 relative">
          ...
          <div className="absolute right-0 inset-y-0 my-[1px] mr-[1px] flex items-center bg-gray-800 px-2 rounded-r-md cursor-default">
            <span
              title={showPassword ? "Hide password" : "Show password"}
              className="p-1 rounded-full overflow-hidden hover:bg-gray-600 cursor-pointer"
              onClick={() => setShowPassword(!showPassword)}
            >
              {showPassword ? <IconSlashEye /> : <IconEye />}
            </span>
